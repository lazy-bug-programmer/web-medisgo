...
const TARGET_HOSPITAL = "Gleneagles Hospital Penang";
...
// Helper function to find doctor in JSON by name
function findDoctorImageUrl(firstName, lastName, doctorsJSON) {
  const fullName = normalizeName(`${firstName} ${lastName}`);

  for (const doctor of doctorsJSON) {
    const jsonName = normalizeName(doctor.displayName || doctor.title || "");

    // Try exact match first
    if (jsonName === fullName) {
      if (doctor.workLocations && doctor.workLocations.length > 0) {
        // Filter for Gleneagles Penang location
        for (const location of doctor.workLocations) {
          if (location.urlName === "gleneagles-hospital-penang") {
            const imageUrl = location.doctorImage;
            if (imageUrl && imageUrl !== "") {
              return imageUrl;
            }
          }
        }
        // Fallback to first location if Penang not found
        const imageUrl = doctor.workLocations[0].doctorImage;
        if (imageUrl && imageUrl !== "") {
          return imageUrl;
        }
      }
  ...
  for (const doctor of doctorsJSON) {
    ...
    if (
      jsonName.includes(normalizedFirst) &&
      jsonName.includes(normalizedLast)
    ) {
      if (doctor.workLocations && doctor.workLocations.length > 0) {
        // Filter for Gleneagles Penang location
        for (const location of doctor.workLocations) {
...
async function downloadAndUploadImage(imageUrl, doctorName) {
  try {
    ...
    const fullUrl = `https://gleneagles.com.my${imageUrl}`;
...
async function importDoctors() {
  ...
  const jsonFilePath = path.join(__dirname, "./doctorsdata-min.json");

  // Read JSON file
  console.log("Reading JSON file...");
  const jsonContent = fs.readFileSync(jsonFilePath, "utf-8");
  const doctorsJSON = JSON.parse(jsonContent);
  console.log(`Found ${doctorsJSON.length} doctors in JSON file`);
  ...
  // Filter doctors from Gleneagles Hospital Penang
  ...
  if (doctors.length === 0) {
    console.log("No doctors found from Gleneagles Hospital Penang. Exiting.");
    return;
  }
  ...
  for (let i = 0; i < doctors.length; i++) {
    ...
    try {
      // Find matching image URL from JSON
