...
const TARGET_HOSPITAL = "ICON ONCOLOGY PENANG";
...
function parseHtmlDoctors(htmlContent) {
  ...
  // Icon Oncology uses doctor-card structure
  // Structure: <img> with doctor image followed by <h4 class="doctor-card-title">Doctor Name</h4>
  // We need to find image-name pairs in the HTML

  // Find all doctor card sections
  const regex =
    /<img[^>]*src="([^"]+)"[^>]*alt="Profile picture of[^>]*>[\s\S]{0,300}?<h4 class="doctor-card-title">([^<]+)<\/h4>/gi;
  let match;
...
async function downloadAndUploadImage(imageUrl, doctorName) {
  try {
    ...
    const fullUrl = imageUrl.startsWith("http")
      ? imageUrl
      : `https://www.icononcology.com.my${imageUrl}`;
