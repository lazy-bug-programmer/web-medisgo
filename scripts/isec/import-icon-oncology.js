...
const TARGET_HOSPITAL = "ISEC Penang";
...
function parseHtmlDoctors(htmlContent) {
  ...
  // ISEC Penang uses kt-blocks-info-box structure
  // Structure: <img> with doctor image followed by <h2 class="kt-blocks-info-box-title">DR. NAME</h2>
  // We need to find image-name pairs in the HTML

  // Find all doctor sections
  const regex =
    /<img[^>]*src="([^"]+)"[^>]*>[\s\S]{0,500}?<h2 class="kt-blocks-info-box-title">([^<]+)<\/h2>/gi;
  let match;
