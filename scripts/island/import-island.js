...
const TARGET_HOSPITAL = "Island Hospital";
...
function parseHtmlDoctors(htmlContent) {
  ...
  // Match doctor grid items
  const gridItemRegex =
    /<div class="vc_grid-item[^"]*"[^>]*>[\s\S]*?<\/div>\s*<div class="vc_clearfix"><\/div>\s*<\/div>/g;
  const items = htmlContent.match(gridItemRegex) || [];

  for (const item of items) {
    // Extract doctor name from title or heading
    const titleMatch = item.match(/title="([^"]+Dr\.)"/);
    const nameMatch = item.match(
      /<a[^>]*class="vc_gitem-link"[^>]*title="([^"]+Dr\.)"[^>]*>([^<]+)<\/a>/
    );

    // Extract image URL
    const imgMatch = item.match(/<img[^>]*src="([^"]+)"[^>]*alt=""/);

    if ((titleMatch || nameMatch) && imgMatch) {
      const doctorName = titleMatch
        ? titleMatch[1]
        : nameMatch
        ? nameMatch[1]
        : null;
...
async function downloadAndUploadImage(imageUrl, doctorName) {
  try {
    ...
    const fullUrl = imageUrl.startsWith("http")
      ? imageUrl
      : `https://islandhospital.com${imageUrl}`;
