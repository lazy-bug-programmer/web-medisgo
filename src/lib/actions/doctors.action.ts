...
export async function createDoctorWithImage(doctor: Omit<Doctor, "$id">, profileImage?: File) {
    try {
...
export async function updateDoctorWithImage(doctorId: string, updates: Partial<Doctor>, newProfileImage?: File) {
    try {
        const user = await getLoggedInUser();
        if (!user) {
            return { error: "Not authorized" };
...
export async function getDoctorsBySpecialty(specialty: number, limit = 10000) {
    try {
        const { databases } = await createClient();
...
export async function getDoctorsByDepartment(department: number, limit = 10000) {
    try {
        const { databases } = await createClient();
...
export async function adminUpdateDoctor(doctorId: string, updates: Partial<Doctor>) {
    try {
        const { databases } = await createAdminClient();
